...
public class DLPluginManager {
	...
	/** 持有所有插件apk的Package信息，键值为包名，value为apk信息，包括指定的类加载器 */
	 ...
	 * 加载插件apk所需运行环境信息,类加载器DexClassLoader,资源管理DexClassLoader等等
	 * 
	 * @param packageInfo
	 * @param dexPath
	 * @return
	...
	private DLPluginPackage preparePluginEnv(PackageInfo packageInfo,
		...
		DLPluginPackage pluginPackage = mPackagesHolder
		...
		pluginPackage = new DLPluginPackage(dexClassLoader, resources,
				packageInfo);
	...
	private DexClassLoader createDexClassLoader(String dexPath) {
		...
		DexClassLoader loader = new DexClassLoader(dexPath, dexOutputPath,
				mNativeLibDir, mContext.getClassLoader());
		return loader;
	}

	/***
	 * 通过反射的方式添加一个指定路径资源管理AssetManager
	 * 
	 * @param dexPath
	 * @return
	 */
	...
	private void copySoLib(String dexPath) {
		...
		SoLibManager.getSoLoader().copyPluginSoLib(mContext, dexPath,
				mNativeLibDir);
	}
	...
	public int startPluginActivityForResult(Context context, DLIntent dlIntent,
			int requestCode) {
		...
		final String className = getPluginActivityFullPath(dlIntent,
				pluginPackage);
	...
	public int startPluginService(final Context context, final DLIntent dlIntent) {
		...
		fetchProxyServiceClass(dlIntent, new OnFetchProxyServiceClass() {
			@Override
			public void onFetch(int result,
					Class<? extends Service> proxyServiceClass) {
	...
	public int bindPluginService(final Context context,
			final DLIntent dlIntent, final ServiceConnection conn,
			final int flags) {
		if (mFrom == DLConstants.FROM_INTERNAL) {
			dlIntent.setClassName(context, dlIntent.getPluginClass());
	...
	public int unBindPluginService(final Context context, DLIntent dlIntent,
	 ...
	 */
	private void fetchProxyServiceClass(DLIntent dlIntent,
			OnFetchProxyServiceClass fetchProxyServiceClass) {
		String packageName = dlIntent.getPluginPackage();
		if (TextUtils.isEmpty(packageName)) {
			throw new NullPointerException("disallow null packageName.");
		}
	...
	private String getPluginActivityFullPath(DLIntent dlIntent,
			DLPluginPackage pluginPackage) {
		String className = dlIntent.getPluginClass();
		className = (className == null ? pluginPackage.defaultActivity
				: className);
	...
	private Class<? extends Activity> getProxyActivityClass(Class<?> clazz) {
		...
		// isAssignableFrom 是否可以从指定类赋值，
		// 如Object.class.isAssignableFrom(String) ,Object可以接收String类型
	...
	private void performStartActivityForResult(Context context,
			DLIntent dlIntent, int requestCode) {
